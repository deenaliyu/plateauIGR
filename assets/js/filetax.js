...
$("#generateReferenceNum").on("click", async function () {
  ...
  const API_KEY = "As4f2CzVQtSIcbX33ur0Xz"; // Replace with your Filestack API Key
  let user_id = "";
  ...
  try {
    let uploadPromises = Array.from(fileInputs).map(async (fileInput) => {
      ...
      formData.append("fileUpload", file); // Ensure correct key name
      formData.append("filename", file.name);
      formData.append("mimetype", file.type);

      let response = await fetch(`https://www.filestackapi.com/api/store/S3?key=${API_KEY}`, {
        method: "POST",
        body: formData
      });
      ...
      if (data.url) {
        obj.data[fileInput.dataset.name] = data.url;
      } else {
        console.error("Filestack upload failed:", data);
