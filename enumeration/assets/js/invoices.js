...
$("#genInvBtn").on("click", function(){
    window.location.href = `../generateinvoice.html?created_by=enumerator&id=${userInfo2?.id}`
})
...
async function fetchInvoice() {
  ...
  const response = await fetch(
    `${HOST}?getAllInvoiceEnumerator&enum_id=${userInfo2?.id}`
  );
...
async function fetchAnalytics() {
  ...
  try {
    const response = await fetch(
      `${HOST}/php/index.php?dashboardAnalyticsEnumerator&enum_id=${userInfo2?.id}`
    );
