...
function loadFacilities() {
  try {
    ...
    let url = `${HOST}?getSchoolFacilities=1`;
    
    // Check if userInfo2 exists (admin view shows all, enumerator shows only theirs)
    if (typeof userInfo2 !== 'undefined' && userInfo2.id) {
      // Only add enumerator filter if user is an enumerator, not admin
      if (userInfo2.role === 'enumerator') {
        url += `&enumerator_id=${userInfo2.id}`;
      }
    }
...
// Render facilities table with admin columns
function renderFacilities(facilities) {
  ...
  facilities.forEach((facility, index) => {
    // Format TIN response status
    const tinStatus = (facility.tin_response || 'no').toUpperCase();
    const tinBadgeClass = tinStatus === 'YES' ? 'bg-success' : 'bg-warning';
    
    // Format date
    const dateCreated = facility.created_at ? facility.created_at.split(" ")[0] : 'N/A';
    ...
    html += `
      <tr>
        ...
        <td><span class="badge ${tinBadgeClass}">${tinStatus}</span></td>
        ...
        <td>
          <div class="btn-group d-flex gap-2">
            ...
            <a class="btn btn-secondary btn-sm print-btn" 
              href="./hospital/enumeration-schools-preview.html?id=${facility.enumeration_id}"
              title="Print">
...
function renderFacilityDetails(facilityData) {
  ...
  // Format TIN status
  const tinStatus = (facilityData.tin_response || 'no').toUpperCase();
  const tinBadgeClass = tinStatus === 'YES' ? 'bg-success' : 'bg-warning';
    ...
    <div class="row">
      <div class="col-md-12 mb-4">
        ...
        <table class="table table-sm table-bordered">
          ...
          <tr>
            <th>Date Created:</th>
            <td>${facilityData.created_at || 'N/A'}</td>
          </tr>
        </table>
      </div>
...
function initializeDataTable() {
  ...
  $('#dataTable').DataTable({
    ...
    responsive: true,
    pageLength: 25,
    order: [[10, 'desc']] // Sort by Date Created column (index 10) descending
  });
}
      ...
      if (data.status === 1 && data.facilities) {
        ...
        const exportData = facilities.map(facility => {
          ...
          let row = {
            ...
            "TIN": facility.tin,
            "TIN Status": (facility.tin_response || 'no').toUpperCase(),
            ...
            "Date Created": facility.created_at
          };
...
function loadSummaryTiles() {
  ...
  fetch(url)
    ...
    .catch(error => {
      console.error('Error loading summary tiles:', error);
    });
}

// Render summary tiles
function renderSummaryTiles(facilities) {
  const summaryContainer = $('#facilityTypeSummary');
  summaryContainer.empty();

  if (!facilities || facilities.length === 0) {
    summaryContainer.html('<div class="col-12 text-center text-muted">No data available</div>');
    return;
  }
  ...
  facilities.forEach(facility => {
    ...
    // Sum students and staff
    if (facility.type_data) {
      const students = parseInt(facility.type_data.avg_number_of_students) || 0;
      const staff = parseInt(facility.type_data.number_of_staff) || 0;
      totalStudents += students;
      totalStaff += staff;
    }

    // Count by LGA
    const lga = facility.lga || 'Unknown';
    ...
    <div class="col-md-3 mb-3">
      <div class="card h-100" >
        <div class="card-body text-white">
          <h6 class="card-title fontBold">TOTAL SCHOOLS</h6>
           <p class="card-text display-4 text-primary">${facilities.length}</p>
        </div>
      </div>
    </div>
  `);
  ...
  summaryContainer.append(`
     ...
     <div class="col-md-3 mb-3">
        <div class="card h-100">
          <div class="card-body">
  <h6 class="card-title fontBold">TOTAL STUDENTS</h6>
 <p class="card-text display-4 text-primary"> ${totalStudents.toLocaleString()}</p>
  ...
  summaryContainer.append(`
    <div class="col-md-3 mb-3">
      <div class="card h-100">
        <div class="card-body text-white">
          <h6 class="card-title fontBold">TOTAL STAFF</h6>
          <p class="display-4 mb-0">${totalStaff.toLocaleString()}</p>
        </div>
      </div>
    </div>
  `);
  ...
  summaryContainer.append(`
    <div class="col-md-3 mb-3">
      <div class="card h-100">
        <div class="card-body text-white">
          <h6 class="card-title fontBold">TOP LGA</h6>
          <p class="h5 mb-0 text-primary">${topLGA ? topLGA[0] : 'N/A'}</p>
          <small>${topLGA ? topLGA[1] + ' schools' : ''}</small>
        </div>
      </div>
    </div>
  `);
  ...
  const sortedTypes = Object.entries(schoolTypes).sort((a, b) => b[1] - a[1]);
  sortedTypes.forEach(([type, count]) => {
    const formattedType = formatFacilityType(type);
    summaryContainer.append(`
      <div class="col-md-3 mb-3">
        <div class="card h-100" style="background: ">
          <div class="card-body text-white">
            <h6 class="card-title fontBold text-uppercase" style="font-size: 12px;">${formattedType}</h6>
                <p class="card-text display-4 text-primary">${count}</p>
            <small>${((count / facilities.length) * 100).toFixed(1)}% of total</small>
          </div>
        </div>
      </div>
    `);
