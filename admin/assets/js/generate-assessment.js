...
async function generateInvoiceNum(taxNumber) {
    ...
    let thesector = document.querySelector("#thesector").value
    ...
    $.ajax({
        type: "GET",
        url: `${HOST}?generateSingleInvoices&tax_number=${taxNumber}&revenue_head_id=${the_id}&price=${amountto.join(',')}&description=${description}&invoice_type=direct&sector=${thesector}`,
        dataType: 'json',
        success: function (data) {
