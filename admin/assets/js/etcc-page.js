async function getEtccRequests() {
    ...
    if (etccReqs.status === 0) {
        ...
        let approvedData = etccReqs.message.reverse().filter(etcc => etcc.app_status === "Accepted")
        let declinedData = etccReqs.message.reverse().filter(etcc => etcc.app_status === "Declined")
        // console.log(approvedData)
        approvedData.forEach((etcReq, i) => {
            $("#etccTable4").append(`
            <tr>
              <td>${i + 1}</td>
              <td>${etcReq.timeIn}</td>
