...
function Profile() {
  ...
  $(".mainInfo").html(`
    <h4 class="text-[18px] text-[#2E2F5B] mt-2">${userInfo.fullname}</h4>
  `)
  $(".prof").html(`
    <div class="flex justify-between md:w-[550px]">
      <label class="w-[195px]">Full Name</label>
      <div class="form-group md:w-[454px] w-full">
        <input class="form-control mt-1 regInputs" readonly type="text" value="${userInfo.fullname}"
          maxlength="15" />
      </div>
    </div>
  ...
  profilo += `
    ...
    <div class="form-group w-8/12">
      <input class="form-control mt-1 updtProf" data-name="fullname" type="text" value="${userInfo.fullname}" maxlength="15" />
    </div>
  </div>
...
async function fetchUserDetails() {
  const response = await fetch(`${HOST}?userProfileAdmin&id=${userDATA.id}`)
